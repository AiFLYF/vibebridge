...
core/vb.py      存储、置信度演算、指标计算、安全闸门、报告生成
```

**你永远不直接编辑任何 JSON 文件。** 所有写入必须走 CLI，它会做 schema 校验、原子写入、备份和日志。

```bash
python "<skill-dir>/core/vb.py" <command> [options]
```
...
```bash
python core/vb.py session close --summary "做完了音乐可视化的频谱部分。"
```
...
```bash
python core/vb.py project create --name "音乐可视化网页" --idea "跟着音乐变化的网页" --stack "html,js"
python core/vb.py project update --id proj_0001 --stage completed --event completed
python core/vb.py project bump --id proj_0001 --field feedback_received --amount 2
```
...
```bash
python core/vb.py experiment propose --objective "为音乐可视化做一个展示页" \
...
```bash
python core/vb.py experiment complete --id experiment_001 --result successful \
...
```bash
python core/vb.py experiment decline --id experiment_001 --reason "现在不想"
```
...
```bash
python core/vb.py profile --strategies
```
...
```bash
python core/vb.py safety --text "<相关内容>"
```
