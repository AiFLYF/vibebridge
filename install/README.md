...
cp install/vb-*.md ~/.claude/commands/
```
...
Copy-Item install\vb-*.md "$HOME\.claude\commands\"
```
...
命令文件里写死了 `$HOME/.claude/skills/vibebridge/core/vb.py`。如果你把 skill 装在别处，改一下这些文件里的路径即可。
