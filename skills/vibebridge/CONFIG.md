...
> 下文 `scripts/vb.py` 指 `<skill-dir>/scripts/vb.py`，`<skill-dir>` 是本 Skill 的安装目录
> （典型值 `~/.claude/skills/vibebridge`）。执行时请展开为绝对路径。

```bash
python scripts/vb.py config
python scripts/vb.py config --set growth.quiet_days_after_declines=14
```
...
```bash
python scripts/vb.py config --set growth.enabled=false
```

想更保守（拒绝一次就静默两周）：

```bash
python scripts/vb.py config --set growth.decline_streak_trigger=1
python scripts/vb.py config --set growth.quiet_days_after_declines=14
```
...
| 常量 | 位置 | 默认 |
|---|---|---|
| `SAME_SESSION_DAMPING` | `scripts/memory.py` | `0.25` |
| `SATURATION_RATE` | `scripts/memory.py` | `0.9` |
| `CONTRADICT_WEIGHT` | `scripts/memory.py` | `1.15` |
| `STABILITY_ON_SUCCESS` | `scripts/growth.py` | `0.35` |
| `PROMOTE_STABILITY` | `scripts/growth.py` | `0.60` |
| `COOLDOWN_HOURS_AFTER_LEVEL_UP` | `scripts/growth.py` | `48` |
| `MIN_CONFIDENCE_FOR_PROFILE` | `scripts/profile.py` | `0.35` |
| `MIN_SAMPLES_FOR_INFERENCE` | `scripts/insight.py` | `4` |
| `FORBIDDEN_PATTERNS` | `scripts/safety.py` | 见 references/safety.md |
